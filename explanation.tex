...
\title{\textbf{Plain-English Companion to\\
       ``Fourier Neural Operators for 2D Navier--Stokes''}}
\author{
  A guide for our own understanding\\
...
\begin{figure}[H]
  ...
  \caption{\textbf{Side-by-side predictions on six test samples.}
           \textbf{Row 1 (Input):} what the model is given --- the current
           vorticity field.
           ...
           U-Net captures the rough shape but blurs out fine details and
           gets the magnitudes slightly wrong (colours are less intense).}
  \label{fig:predictions}
\end{figure}

\subsection{Where the Errors Are}

Figure~\ref{fig:error_maps} shows \emph{where} each model makes mistakes, not
just how much.
Bright = big error; dark = small error.

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{figures/error_maps.png}
  \caption{\textbf{Error maps: where each model goes wrong.}
           ...
           This tells us U-Net struggles most where the turbulence is most
           intense.}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{figures/error_histogram.png}
  \caption{\textbf{Error distribution across the full test set.}
           Each bar counts how many test samples fell in that error range.
           \textbf{Blue (FNO):} most samples clustered tightly near 0.035.
           FNO is consistently good.
           \textbf{Red (U-Net):} spread much wider, with many samples having
           much higher error.
           Not only is U-Net less accurate on average, it is also less
           \emph{reliable} --- some samples it handles fine, others it gets
           badly wrong.}
...
FNO's error is \textbf{literally identical} across all three resolutions (0.019
each time, rounded to three decimal places).
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{figures/super_res.png}
  \caption{\textbf{Visual super-resolution comparison on one test sample.}
           \textbf{Columns:} $64\times64$, $128\times128$ (training res),
           $256\times256$.
           \textbf{Rows:} ground truth / FNO prediction / U-Net prediction.
           FNO looks sharp and accurate at every resolution.
           U-Net at $64\times64$ looks blurry and washed out.
           U-Net at $256\times256$ develops visible artefacts and the colours
           are wrong --- it has no idea what to do at this resolution.}
  \label{fig:super_res}
\end{figure}
...
$1/256$.
The same kernel now covers half the physical area.
Every single learned filter has a different physical meaning at the new
resolution --- which is why U-Net breaks.
...
\begin{itemize}
  ...
  \item \textbf{Single-step only.}
        The model predicts one timestep at a time.
        If you wanted to simulate 100 timesteps into the future, you would need
        to feed the output back as input 100 times.
        Small errors would accumulate --- we did not test this.

  \item \textbf{The super-resolution ``ground truth'' is approximate.}
        At $256 \times 256$, we do not have real simulation data.
        We created it by bilinearly upsampling the $128 \times 128$ data.
        This is a reasonable proxy but not perfect.
\end{itemize}
...
\begin{enumerate}
  ...
  \item FNO is more accurate, faster, smaller, and more flexible than a
        standard CNN (U-Net) on this task.
...
FNO is not a replacement for numerical simulation in all cases, but it is a
compelling tool for cases where you need fast approximate predictions ---
weather nowcasting, real-time flow control, and interactive engineering design
are all natural applications.
...
\begin{description}
  ...
  \item[PDE (Partial Differential Equation)] An equation involving a function
        ...
        Describes how something (temperature, velocity, vorticity) changes
        across both space and time simultaneously.
  ...
  \item[Relative $L^2$ error] $\|\text{prediction} - \text{truth}\|_2 /
        \|\text{truth}\|_2$. A scale-free error metric. Score of 1.0 = random
        guess; 0.0 = perfect.
  ...
  \item[Parameters / weights] The numbers inside a neural network that are
        adjusted during training. More parameters = more capacity (not always
        better).
