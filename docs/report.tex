...
\begin{figure}[t]
  ...
  \caption{Six test samples ordered from lowest to highest FNO error.
           ...
           See Appendix~\ref{app:figures} for enlarged version.}
...
The figures below are enlarged versions of those in the main body, included
for ease of inspection.

\clearpage
\begin{figure*}[p]
  \centering
  \includegraphics[width=\textwidth]{sample_pairs.png}
  \caption{(Enlarged) Five input--target vorticity pairs from the training set.
           Top row: input $\omega(t)$.
           Bottom row: target $\omega(t+1)$.}
\end{figure*}
...
\begin{figure*}[p]
  ...
  \caption{(Enlarged) Six test samples ordered from lowest to highest FNO error.
           Rows: input $\omega(t)$ / FNO prediction / U-Net prediction / ground
           truth $\omega(t+1)$.}
\end{figure*}
...
\begin{figure*}[p]
  ...
  \caption{(Enlarged) Absolute prediction error $|\hat{u} - u|$ for FNO (top)
           and U-Net (bottom).
           Color scale: hot (black = zero, white = maximum).}
\end{figure*}
...
\begin{figure*}[p]
  \centering
  \includegraphics[width=0.8\textwidth]{error_histogram.png}
  \caption{(Enlarged) Distribution of per-sample relative $L^2$ error over the
           full test set.}
\end{figure*}
...
\begin{figure*}[p]
  ...
  \caption{(Enlarged) Zero-shot super-resolution on a single test sample.
           Columns: 64$\times$64, 128$\times$128 (train resolution), 256$\times$256.
           Rows: ground truth / FNO prediction / U-Net prediction.}
\end{figure*}
